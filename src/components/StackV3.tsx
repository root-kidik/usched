...
export interface StackV3Props extends RectProps {
    title: string;
    capacity: string; // Добавлено
    amount: number;
    blockWidth: number;
    blockHeight: number;
    color: string;
    stroke_color: string;
    top_address: string; // Добавлено
    bot_address: string; // Добавлено
}
                ...
                <Rect direction={'column'} gap={20} alignItems={'center'}>
                    ...
                    <Layout gap={10} direction={'column'} alignItems={'center'}>
                        {range(props.amount).map(i => {
                            ...
                            // Используем setTimeout или логику инициализации после рендера, 
                            // но в рамках конструктора Motion Canvas это работает так:
